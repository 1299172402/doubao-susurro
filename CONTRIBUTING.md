...
1. 解析命令行参数（`-silent` 静默模式、`-config` 指定配置文件路径）
...
- **开机自启** — 复选框，切换 `startup` 配置并安装/卸载开机自启脚本
- **设置** — 打开浏览器配置页面
- **退出** — 退出程序
...
| `/` | GET | Web 设置界面 |
| `/logo.png` | GET | 应用图标 |
| `/donate/wechat.png` | GET | 微信收款码 |
| `/donate/alipay.jpg` | GET | 支付宝收款码 |
...
- 不采用轮询，而是通过长连接(ws)主动触发API请求，豆包在 SharedWorker 中为多个标签页共享后台线程，可在 `chrome://inspect/#workers` 中查看，但目前一方面对于用户操作步骤更多，另一方面目前对于这个 wss 地址本身的连接测试并不成功。
